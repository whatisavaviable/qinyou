...
{
	TimeDelayDataReader *UserDataType_reader = NULL;
	TimeDelaySeq data_seq;
	SampleInfoSeq info_seq;
	ReturnCode_t retcode;
	int i;

	/* 利用reader，创建一个读取UserDataType类型的UserDataType_reader*/
	UserDataType_reader = TimeDelayDataReader::narrow(reader);
	...
	//	TimeDelayTypeSupport::print_data(&data_seq[i]);
	//}
}
...
extern "C" int publisher_main(int domainId, int sample_count)
{
	...
	TimeDelayDataWriter * UserDataType_writer = NULL;
	TimeDelay *instance = NULL;
	ReturnCode_t retcode;
	...
	/* 建议1：在程序启动后优先进行注册 */
	/* 建议2：一个数据类型注册一次即可 */
	type_name = TimeDelayTypeSupport::get_type_name();
	retcode = TimeDelayTypeSupport::register_type(
	...
	UserDataType_writer = TimeDelayDataWriter::narrow(writer);
	...
	instance = TimeDelayTypeSupport::create_data();
	if (instance == NULL) {
	...
	scanf("%d", &times);//设置循环次数

	DWORD time1 = time(NULL);//开始时间的秒数
	...
	DWORD time2 = time(NULL);//结束时间的秒数

	printf("TimeDelay:%f\n", ((double)time2 - (double)time1)/times/2);
	/* 8. 删除数据样本 */
	retcode = TimeDelayTypeSupport::delete_data(instance);
	if (retcode != RETCODE_OK) {
