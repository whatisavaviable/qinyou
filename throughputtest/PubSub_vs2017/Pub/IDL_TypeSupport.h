...
public:
	static ThroughPut* create_data()
	{
		...
		cin >> data;
		getchar();
		return new ThroughPut(data);//输入要传输的数据
	}
	...
	static void print_data(ThroughPut* pThroughPut,int layer = 0)
	{
		...
		c[i] = '\0';
		std::cout<<c<<"ThroughPut:"<<std::endl;
		std::cout<<c<<"  num = "<< pThroughPut->num <<std::endl;
		std::cout<<c<<"  data = "<< pThroughPut->data <<std::endl;
	}
