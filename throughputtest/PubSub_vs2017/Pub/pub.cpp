...
extern "C" int publisher_main(int domainId, int sample_count)
{
	...
	/* 建议1：在程序启动后优先进行注册 */
	/* 建议2：一个数据类型注册一次即可 */
	type_name = ThroughPutTypeSupport::get_type_name();
	retcode = ThroughPutTypeSupport::register_type(
	...
	instance = ThroughPutTypeSupport::create_data();//输入instance中存储要传输的数据
	if (instance == NULL) {
	...
	static DWORD time1 = time(NULL);//开始时间的秒数
	int collection = 0;//传输次数

	/* 7. 主循环 ，发送数据 */
	for (count = 0; (sample_count == 0) || (count < sample_count); ++count) {
		collection++;
		instance->num+=1;//每次传输包的序号加一
		DWORD time2=time(NULL);
		if ((time2 - time1) >= 1) {//每过一秒
			time1 = time2;
			printf("ThroughPut:");
			printf("%d\n", collection*8*instance->StructSize());//每秒吞吐量
			collection = 0;
		}
		retcode = ThroughPut_writer->write(*instance, instance_handle);
		if (retcode != RETCODE_OK) {
	...
	/* 8. 删除数据样本 */
	retcode = ThroughPutTypeSupport::delete_data(instance);
	if (retcode != RETCODE_OK) {
