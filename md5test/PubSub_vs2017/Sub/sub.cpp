...
{
	Md5testDataReader *Md5test_reader = NULL;
	Md5testSeq data_seq;
	...
	Md5test_reader = Md5testDataReader::narrow(reader);
	if (Md5test_reader == NULL) {
		fprintf(stderr, "DataReader narrow error\n");
		return;
	}

	/* 获取数据，存放至data_seq，data_seq是一个队列 */
	retcode = Md5test_reader->read(
	...
	for (i = 0; i < data_seq.length(); ++i) {
		Md5testTypeSupport::print_data(&data_seq[i]);
	}
}
